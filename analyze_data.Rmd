...
This research was approved on May 2, 2019, by the Scientific Ethical Committee for Health Sciences of the Pontificia Universidad Católica de Chile (n° 180823024).
...
In the initial phase of the project, several assessment instruments were administered that were subsequently revised. The following modifications were implemented:

1) The study initially focused on patients with major depressive disorder, which served as an inclusion criterion and was determined using the MINI diagnostic interview. This criterion was later removed; consequently, the MINI interview was no longer administered.

2) To assess depressive symptoms, the BDI self-report questionnaire was replaced with the PHQ-9 self-report questionnaire.

3) To assess personality, the IPO self-report questionnaire was replaced with the LPFS-BF-2.0 self-report questionnaire. 

Therefore, the database may contain participants who have completed the MINI, BDI, and IPO questionnaires.
...
The heterogeneity of depressive presentations may constitute a factor that dilutes treatment effectiveness. One of the forms of heterogeneity corresponds to a subtype of patients who have Early Life Maltreatment and Adversity (ELMA), which is reported in 54% of patients with a diagnosis of depression. Individuals with ELMA present with higher rates of depression, have early onset, more severity, comorbidity, and suicide risk, and a smaller response rate.Local research has shown that it is necessary to focus on ELMA in the treatment of depression in patients with histories of ELMA, and that psychoeducation increases patients´ willingness to discuss ELMA with their therapists. The proposed research is an observational case-match study that will evaluate the acceptability and effectiveness of providing trauma-informed psychotherapy, with automatated feedback to patients and therapists regarding ELMA in depressed patients.Outcomes in depression (PHQ-9) and psychological problems (CORE-OM) from 60 patients treated a the PsiConecta Clinic with a trauma-informed treatment and automated feedback will be compared with outcomes of 200 match-cases from other five clinics treated with TAU (from our ongoing Fondecyt project on alliance in depressed patients).

### Theoretical Summary

Worldwide, the lifetime prevalence of Major Depressive Disorder is 10% to 15% of the  population (Lépine & Briley, 2011), constituting the third cause of morbidity, and accounting  for 4.3% of the global burden of illness (WHO, 2011). In Chile, the prevalence of symptoms of depression is 17% in the general population and 26% among women (MINSAL, 2010).  

Treating depression is a priority due to the high subjective, social, and economic burden of the  illness. However, treatment effectiveness is still somewhat disappointing (Craighead &  Dunlop, 2014). The heterogeneity of depressive presentations may constitute a factor that  dilutes treatment effectiveness (Fried, 2016). One of the forms of heterogeneity corresponds to a subtype of patients who have __early life maltreatment and adversity (ELMA)__ (Nanni et al., 2012).  

Childhood maltreatment is reported in 46% of patients with a diagnosis of depression (Nelson  et al., 2016), and it is associated with attachment deficits (Egeland & Sroufe, 1981), impaired  cognitive and affective functioning (Pechtel & Pizzagalli, 2011), and immune dysregulation  (Fagundes et al., 2013). Individuals with ELMA present with higher rates of depression, have  early onset, more severity, comorbidity, and suicide risk, and a smaller response rate (Teicher  & Samson, 2013). Parental experience of maltreatment as a child is one of the most important  predictors of child maltreatment (van IJzendoorn et al., 2020), and thus, working with parents’  own experiences of maltreatment helps stop the intergenerational transmission of violence (Lieberman et al., 2005). 

In Chile, Vitriol et al. (2010) has shown the importance of focusing on ELMA in the treatment  of women with severe depression with histories of ELMA. Also in our country, research has shown that patients are willing to report ELMA from the beginning of psychotherapy (Israel, 2019), especially if they receive an explanation that doing so is important for their diagnosis  and treatment (Correa, 2017). 

The proposed research is an observational case-match study that will evaluate the acceptability and effectiveness of providing __trauma-informed psychotherapy__, with automated feedback to patients and therapists regarding __ELMA__, in __depressed patients__.

### Ethics Committee Approval

This study was incorporated as an amendment to the ethical approval of the FONDECYT 1191299. This FONDECYT 1191299 was approved on May 2, 2019, by the Scientific Ethical Committee for Health Sciences of the Pontificia Universidad Católica de Chile (n° 180823024).

The amendment to include this study was approved on October 5, 2023.

### Specific Objectives of the Study

The __general objective__ of this study is to evaluate the acceptability and effectiveness of  providing trauma-informed psychotherapy, with automated feedback to patients and  therapists regarding ELMA, in depressed patients. 

The __specific objectives are__:

- Assess the __acceptability__ for patients and therapists of the automated ELMA feedback.

- Assess the __effectiveness__ of the trauma-focused psychotherapy providing patients and therapists with ELMA feedback.

- Assess if ELMA __moderates__ which patients benefit most from this intervention. 

### Specific Hypotheses of the Study

- The __ELMA automated feedback__ will be considered __acceptable for patientsand therapists__.

- Patients in the condition that receives __trauma-informed psychotherapy__ will have __greater rates of improvements__ in their depressive symptomatology and overall psychological well being.

- Patients with __greater ELMA__ will __benefit more from trauma-informed psychotherapy__.
...
The procedure is the same as that described for the FONDECYT 1191299. The additional steps included in this study are:

1) Patients receive automated feedback on ELMA based on their CTQ-SF results. This feedback includes automated feedback on the presence of ELMA, psychoeducation on how ELMA may relate to depression and other presenting problems, and the importance of openly discussing ELMA during psychotherapy.

2) After receiving the feedback, patients complete a self-report questionnaire to assess the acceptability of the feedback and how these tools can be improved.
...
The procedure is the same as that described for the FONDECYT project. The additional steps included in this study are:

1) Therapists receive automated feedback on ELMA from their patients based on their CTQ-SF results.This feedback includes automated feedback on the presence of ELMA and training on working effectively with ELMA. 
...
# Create schema-driven column documentation
base_descriptions <- list(
  ...
  "subject_study_site" = "Human-readable study site name for the research subject",
...
# Build a canonical column list so expected columns are documented even if absent in this file
expected_columns <- names(base_descriptions)
extra_columns <- setdiff(names(data), expected_columns)
documented_columns <- c(expected_columns, extra_columns)

# Build column info
column_info <- data.frame(
  Column = documented_columns,
  Type = sapply(documented_columns, function(col) {
  ...
  Description = sapply(documented_columns, function(col) {
  ...
  Sample_Values = sapply(documented_columns, function(col) {
    if (!(col %in% names(data))) {
      return("Not available in current file")
    }

    col_data <- data[[col]]

    # Special handling for factors to show observed levels
    if (is.factor(col_data)) {
      ...
      # For character fields, skip empty strings so examples are informative
      if (is.character(non_missing)) {
        non_missing <- non_missing[nzchar(trimws(non_missing))]
      }

      sample_vals <- unique(non_missing)

      if (length(sample_vals) == 0) {
        return("All values missing")
      }

      sample_vals <- head(sample_vals, 3)
...
```{r ensure-measure-metadata}
# Ensure measure_metadata is available (load if missing)
if (!exists("measure_metadata")) {
  measure_path <- "data/measure_metadata.csv"
  if (file.exists(measure_path)) {
    measure_metadata <- readr::read_csv(measure_path, show_col_types = FALSE)
  } else {
    warning("measure_metadata.csv not found; measure tables will be empty.")
    measure_metadata <- tibble::tibble(
      measure_id = integer(),
      measure_title = character(),
      measure_label = character(),
      measure_description = character()
    )
  }
}
```
...
datatable(all_measures, 
          escape = c(1, 2, 3),  # render HTML only in Description
          options = list(pageLength = 20, scrollX = TRUE),
...
## Wide Patient-Item Matrix (Patients as Rows)

This section constructs a wide-format representation where each row is a patient and each column is a specific item response from a specific administration of a measure. Column naming follows the pattern:

- Naming: `MEASURECODE_ITEMPOSITION_tN`
- Example: `PHQ_9_1_t1`, `PHQ_9_2_t1`, ..., `PHQ_9_1_t2` for a patient with two PHQ-9 administrations.

Rules:
- Subject: Only patients are included (subject_type = "Patient").
- Ordering: Administrations are ordered by `admin_start_datetime` per subject × measure; items ordered by `item_position` within each administration.
- Values: Numeric responses when available; otherwise character responses. All values stored as text in the wide CSV.

```{r wide-matrix-build, message=TRUE}
# Ensure measure metadata available
if (!exists("measure_metadata")) {
  measure_path <- "data/measure_metadata.csv"
  if (file.exists(measure_path)) {
    measure_metadata <- readr::read_csv(measure_path, show_col_types = FALSE)
  } else {
    stop("measure_metadata.csv not found; cannot build wide matrix")
  }
}

# Helper to build a measure code from label/title
sanitize_code <- function(x) {
...
# Measure codes lookup
measure_codes <- measure_metadata %>%
  transmute(
    item_measure_id = measure_id,
    measure_code = dplyr::coalesce(sanitize_code(measure_label), sanitize_code(measure_title), paste0("MEASURE_", measure_id))
  )

patients_long <- data %>%
  filter(subject_type == "Patient", !is.na(item_id), !is.na(item_measure_id)) %>%
  # admin sequence per subject x measure
  mutate(admin_start_datetime = as.POSIXct(admin_start_datetime, tz = "UTC")) %>%
  distinct(subject_id, item_measure_id, administration_id, admin_start_datetime) %>%
  arrange(subject_id, item_measure_id, admin_start_datetime, administration_id) %>%
  group_by(subject_id, item_measure_id) %>%
  mutate(admin_seq = row_number()) %>%
  ungroup() %>%
  # join back to all responses to get items
  right_join(
    data %>% filter(subject_type == "Patient", !is.na(item_id), !is.na(item_measure_id)) %>%
      select(subject_id, item_measure_id, administration_id, response_id, response_date, item_position, item_label, numeric_value, character_value),
    by = c("subject_id", "item_measure_id", "administration_id")
  ) %>%
  left_join(measure_codes, by = "item_measure_id")

patients_long <- patients_long %>%
  mutate(
    item_pos = suppressWarnings(as.integer(item_position)),
    item_pos = ifelse(is.na(item_pos), 9999L, item_pos),
    measure_code = ifelse(is.na(measure_code) | measure_code == "", paste0("MEASURE_", item_measure_id), measure_code),
    col_name = paste0(measure_code, "_", item_pos, "_t", admin_seq),
    value = ifelse(!is.na(numeric_value), as.character(numeric_value), as.character(character_value)),
    resp_time = suppressWarnings(as.POSIXct(response_date, tz = "UTC"))
  )

# Pick the latest response if duplicates exist for a subject + col_name
patients_long_unique <- patients_long %>%
  filter(!is.na(col_name)) %>%
  group_by(subject_id, col_name) %>%
  arrange(resp_time, response_id, .by_group = TRUE) %>%
  summarise(value = dplyr::last(value), .groups = "drop")

patients_wide <- patients_long_unique %>%
  tidyr::pivot_wider(names_from = col_name, values_from = value)

# Reorder columns: by measure_code, then administration sequence, then item position
col_structure <- patients_long %>%
  filter(!is.na(col_name)) %>%
  distinct(measure_code, admin_seq, item_pos, col_name) %>%
  arrange(measure_code, admin_seq, item_pos)

col_order <- col_structure$col_name

patients_wide <- patients_wide %>%
  dplyr::select(subject_id, dplyr::all_of(col_order))

# Save wide matrix and a variable dictionary
wide_csv <- "data/patient_item_matrix_wide_202507271125.csv"
readr::write_csv(patients_wide, wide_csv)

wide_dict <- patients_long %>%
  transmute(
  ...
  distinct(column_name, item_measure_id, measure_code, item_position, admin_seq)

# Enhance dictionary with measure + item info
item_lookup <- data %>%
  select(item_id, item_label, item_position, item_measure_id) %>%
  distinct()

wide_dict <- patients_long %>%
  select(col_name, item_measure_id, item_position = item_pos, admin_seq, item_label) %>%
  distinct() %>%
  left_join(measure_metadata %>% select(measure_id, measure_title, measure_label), by = c("item_measure_id" = "measure_id")) %>%
  left_join(measure_codes, by = "item_measure_id") %>%
  transmute(
    column_name = col_name,
  ...
  arrange(measure_code, admin_seq, item_position)

wide_dict_csv <- "data/patient_item_matrix_wide_dictionary_202507271125.csv"
readr::write_csv(wide_dict, wide_dict_csv)

cat("\n✅ Wide matrix saved to:", wide_csv, "\n")
cat("📘 Dictionary saved to:", wide_dict_csv, "\n")
```

### Wide Matrix Summary {.alert .alert-info}

```{r wide-summary, results='asis'}
cat("- Rows (patients): ", nrow(patients_wide), "\n", sep = "")
cat("- Columns (responses): ", ncol(patients_wide) - 1, "\n", sep = "")
cat("- Example columns: ", paste(head(setdiff(names(patients_wide), "subject_id"), 8), collapse = ", "), "\n", sep = "")
```

### Wide Matrix Preview

```{r wide-preview}
preview_wide <- patients_wide %>%
  select(subject_id, head(setdiff(names(patients_wide), "subject_id"), 25))

datatable(preview_wide,
          options = list(pageLength = 5, scrollX = TRUE),
          caption = "Preview: Patient × Item Responses (Wide)",
          rownames = FALSE)
```
...
```{r wide-dict-preview}
dict_preview <- wide_dict %>% head(50)
knitr::kable(dict_preview, align = 'l') %>%
